...
const spawnChecked = (
    ...
    child.on("exit", (code: number | null, signal: NodeJS.Signals | null) => {
        ...
        if (code && code !== 0) {
            console.error(`[orbit-dev] ${options.name} exited with code ${code}`);
    ...
    child.on("error", (error: Error) => {
        console.error(`[orbit-dev] Failed to start ${options.name}: ${error.message}`);
        process.exitCode = 1;
        options.onFailure?.();
    });
...
const run = async () => {
    const assertPortFree = (
        ...
        new Promise<void>((resolve) => {
            ...
            tester.on("error", () => {
                console.error(
                    `[orbit-dev] Port ${port} is already in use on ${host}. Set ${envVar} to a free port and retry.`,
                );
                process.exitCode = 1;
                resolve();
            });
    ...
    const shutdownChildren = (reason: string) => {
        ...
        shuttingDown = true;
        console.log(`[orbit-dev] Shutting down (${reason})...`);
    ...
    children.push(
        spawnChecked(
            nextBin,
            ["dev", "-p", ORBIT_NEXT_PORT, "-H", "127.0.0.1"],
            {
                name: "next",
                env: {
    ...
    children.push(
        spawnChecked(tsxBin, ["watch", "middle/server.ts"], {
            name: "middle",
            env: {
    ...
    console.log("");
    console.log(`[orbit-dev] Admin address: https://127.0.0.1:${ORBIT_HTTPS_PORT}`);
    console.log(`[orbit-dev] Guest address: https://${entrypointHost}:${ORBIT_HTTPS_PORT}`);
    if (lanIps.length > 1) {
        console.log(`[orbit-dev] LAN IPv4 addresses: ${lanIps.join(", ")}`);
    }
    console.log(`[orbit-dev] HTTP redirect: http://${entrypointHost}:${ORBIT_HTTP_PORT} -> https://${entrypointHost}:${ORBIT_HTTPS_PORT}`);
    console.log(`[orbit-dev] Next (internal): http://127.0.0.1:${ORBIT_NEXT_PORT}`);
    console.log(`[orbit-dev] Middle (internal): http://127.0.0.1:${ORBIT_MIDDLE_PORT}`);
    console.log(`[orbit-dev] Caddy root CA (for iOS trust): ${rootCaPath}`);
    console.log("");
